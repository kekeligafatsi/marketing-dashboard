...
- Optional sales-rep login (`supabase.auth.signInWithPassword`/`getSession`/`onAuthStateChange`) via a new "Sales Rep Sign In" control in the top bar. A signed-in rep gets new leads auto-attributed to them (`assignedToId`, stamped from their session, alongside the existing `loggedBy` display name), sees only their own leads across Dashboard/Pipeline/History, and can only edit/delete their own entries. Owners keep the existing no-login view unchanged and gain a "filter by rep" dropdown (Pipeline toolbar) plus rep-scoped stat labels for reviewing any one rep's numbers. This is client-side attribution/filtering only, not a security boundary — see [ARCHITECTURE.md](ARCHITECTURE.md) and the README's "Sales rep login" section.
- Role-based monthly targets: the single shared "Monthly Revenue Target"/"Monthly Lead Target" cards are replaced by a "Sales Manager Target" and a "Sales Officer Target" card, each with its own revenue and lead-count input and progress bar. A rep's role (`sales_manager`/`sales_officer`) comes from their Supabase Auth `user_metadata.role` and is stamped onto each of their entries as `assignedToRole` (same pattern as `assignedToId`). A signed-in rep sees only their own progress against their role's target; the owner sees each role's combined progress by default, narrowed to one person via the existing "filter by rep" dropdown. Existing single-target data is migrated automatically — both roles are seeded from the old shared number the first time it's loaded, so no target is silently reset. See the README's "Role-based revenue and lead targets" section.
