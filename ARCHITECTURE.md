...
4. **Sync + conflict check** (`persistToSupabase()`, `index.html:318-349`): before writing, it re-reads the row's `updated_at` and compares it to the last value this tab observed (`lastKnownRemoteUpdatedAt`). If a teammate's tab wrote more recently, this tab **discards its own pending write**, pulls the newer remote state, re-renders, and shows a toast asking the user to redo their edit — rather than silently clobbering someone else's change. Otherwise it `upsert`s the whole state blob back to the single shared row. This check is whole-document, not per-entry, so with sales reps logged in and editing concurrently alongside owners, "someone else updated the dashboard, please redo your edit" toasts will fire more often than with a single loosely-coordinated editor — including when the concurrent edit touched a completely different rep's unrelated lead. Worth keeping in mind as usage grows; not something this feature attempts to fix.
...
- **Client-side trust boundary.** The Supabase anon key is public (it ships in the HTML — see [TECHSTACK.md](TECHSTACK.md#backend--services)). Because anyone with the link (or the key) can write to the row, all state loaded from Supabase or `localStorage` is treated as untrusted and passed through `sanitizeState`/`sanitizeEntry`, and all user-supplied strings are escaped (`escapeHtml`) before being interpolated into rendered HTML. There is no RLS configured for this app — access control, if any, would need to live in Supabase Row Level Security policy configuration, which is not part of this file.
- **Sales rep login is identity, not access control.** A rep can optionally sign in via Supabase Auth (`supabase.auth.signInWithPassword`), which sets a client-side `currentUser` used to default the dashboard to that rep's own leads (`assignedToId` field on each entry) and to hide edit/delete affordances on other reps' rows. This is purely a UX/attribution feature: because storage is still the single shared `dashboard_state` row (see above), a signed-in session grants no additional or reduced access at the database level — the anon key can still read/write everything regardless of who's signed in, exactly as before this feature existed. Adding `assignedToId` required no schema migration, since the whole entry already lives inside the one JSON blob.
- **Role is denormalized onto each entry, because the client can't look up other users.** A rep's role (`sales_manager`/`sales_officer`) lives in their own Supabase Auth `user_metadata`, but the anon key has no admin API access to enumerate or read *other* users' metadata — only `supabase.auth.getSession()`/`onAuthStateChange` for whoever is currently signed in. To let the owner (who is never signed in) see a role's combined progress, each entry is stamped with `assignedToRole` at creation time from the creating rep's own `currentUser.role` — the same pattern already used for `assignedToId`/`loggedBy`. `state.targets = { sales_manager: {revenue, leads}, sales_officer: {revenue, leads} }` replaces the old single `revenueTarget`/`leadTarget` numbers; `sanitizeState` seeds both roles from the old single values the first time it encounters a pre-migration document, so an owner's already-configured target isn't silently reset.
